...
echo "******************************************************************"
echo " HELIOS base with FCPortugal Setplays"
echo " Adapted by Luis Mota 2013"
echo "******************************************************************"
...
if [ $number -gt 0 ]; then
  ...
  if  [ X"${offline_mode}" != X'' ]; then
    ...
    echo "Launching player $i:$player ${opt} -g $sleepprog $goaliesleep"
    $player ${opt} -g &
    $sleepprog $goaliesleep
  fi
fi
...
while [ $i -le ${number} ] ; do
  ...
  if  [ X"${offline_mode}" != X'' ]; then
    ...
    echo "Launching player $i:$player ${opt} $sleepprog $sleeptime"
    $player ${opt} &
    $sleepprog $sleeptime
  fi
