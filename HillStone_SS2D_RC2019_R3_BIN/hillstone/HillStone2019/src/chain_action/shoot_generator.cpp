                             ...
                             const Vector2D & target_point )
{
    ...
    if ( goalie
         && 5 < goalie->posCount() //5→
         && goalie->posCount() < 10   //30→10
         && wm.dirCount( ball_move_angle ) > 2 ) //3→2
    {
#ifdef DEBUG_PRINT
        dlog.addText( Logger::SHOOT,
                             ...
                             const double & ball_move_dist )
{
    ...
    const double opponent_x_thr = SP.theirPenaltyAreaLineX() - 50.0;//30→50
          ...
          ++o )
    {
        ...
        if ( (*o)->posCount() > 5 ) continue; //10→5
        if ( (*o)->isGhost() && (*o)->posCount() > 3 ) continue; //5→3
