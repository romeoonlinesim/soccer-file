...
#include "bhv_basic_move_SideHalf.h"
 ...
 */
bool
Bhv_BasicMove_SideHalf::execute( PlayerAgent * agent )
{
...
const PlayerPtrCont & teammates = wm.teammatesFromBall();
    ...
    const double dash_power =  Bhv_BasicMove_SideHalf::getDashPower( agent );
...
if (M_fastest_teammate || M_fastest_teammate->unum() == 10,11,8,9 && M_second_teammate->pos().x,M_second_teammate->pos().y)
	{
		 dlog.addText( Logger::TEAM,
                  __FILE__": 1_2pass_1" );
...
if (M_second_teammate || M_second_teammate->unum() == 10,11,8,9 &&  M_fastest_teammate->pos().x,M_fastest_teammate->pos().y)
	{
...
double Bhv_BasicMove_SideHalf::getDashPower( const PlayerAgent * agent)
 {		//4/12追記,11/16追記
