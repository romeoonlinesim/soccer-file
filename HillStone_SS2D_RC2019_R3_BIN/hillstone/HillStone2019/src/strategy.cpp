...
//const std::string Strategy::DEFENSE_FORMATION_CONF1 = "defense-formation.conf1";
//const std::string Strategy::DEFENSE_FORMATION_KU_CONF = "defense-formation_KU.conf";
//const std::string Strategy::OFFENSE_FORMATION_KU_CONF = "offense-formation_KU.conf";
...
Strategy::updateSituation( const WorldModel & wm )
{
    ...
    int self_min = wm.interceptTable()->selfReachCycle();            //自分が後何サイクルでボールがとれるか
    int mate_min = wm.interceptTable()->teammateReachCycle();        //チームメイトが後何サイクルでボールがとれるか
    int opp_min = wm.interceptTable()->opponentReachCycle();         //相手が後何サイクルでボールを取りにくるか
    int our_min = std::min( self_min, mate_min );

    if ( (opp_min <= our_min -40) && (wm.ball().pos().x < -30) )              //feditのシチュエーションチェンジdeffence 4/16変更　-20->0->-30
    {
        dlog.addText( Logger::TEAM,
    ...
    else if ( (our_min <= opp_min  - 20) && (wm.ball().pos().x > 10) )             //feditのシチュエーションチェンジoffence 4/16変更  -20
    {
        dlog.addText( Logger::TEAM,
...
Strategy::get_normal_dash_power( const WorldModel & wm )
{
    ...
    if ( wm.ourDefenseLineX() > wm.self().pos().x                    //ディフェンスラインの位置が自分のいるポジションより大きいか
         && wm.ball().pos().x < wm.ourDefenseLineX() + 20.0 )        //ボールの位置がディフェンスラインよりも＋２０以下に迫っている時
    {
        dlog.addText( Logger::TEAM,
                      __FILE__": (get_normal_dash_power) correct DF line. keep max power" );
        // keep max power
        dash_power = ServerParam::i().maxDashPower();                //全力でダッシュする
    }
    else if ( s_recover_mode )                                      //ダッシュせずに回復
    {
        dash_power = my_inc - 25.0;                                 // preffered recover value→回復するようにする              
        if ( dash_power < 0.0 ) dash_power = 0.0;                  //まったく走らない

        dlog.addText( Logger::TEAM,
                      __FILE__": (get_normal_dash_power) recovering" );
    }
    // exist kickable teammate
    else if ( wm.existKickableTeammate()                  //見方がボールを持っているときか
              && wm.ball().distFromSelf() < 20.0 )         //自分とボールとの距離が近いとき２０以下の時
    {
        dash_power = std::min( my_inc * 1.1,              /*自分のチームがボールを持っているときにスタミナ（my_inc）×1.1*/
    ...
    // in offside area
    else if ( wm.self().pos().x > wm.offsideLineX() )    //自分の位置がオフサイドラインよりも大きい時
    {
        dash_power = ServerParam::i().maxDashPower();     //ダッシュで戻る
        dlog.addText( Logger::TEAM,
                      __FILE__": in offside area. dash_power=%.1f",
                      dash_power );
    }
    else if ( wm.ball().pos().x > 25.0                         //ボールの位置が２５以上か                  
              && wm.ball().pos().x > wm.self().pos().x + 10.0  //ボールのポジションが自分のポジションより１０先か
              && self_min < opp_min - 8			       //敵が自分のボールを取りにくる6サイクル前より小さい時か
              && mate_min < opp_min - 6 )		       //チームメイトに敵がボールを取りにくる６サイクル前より小さいときで
    {
