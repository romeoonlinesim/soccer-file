...
player="${DIR}/../../binaries/ITAndroids_player"
coach="${DIR}/../../binaries/ITAndroids_coach"
teamname="ITAndroids2018"
host="localhost"
port=6000
coach_port=""
debug_server_host=""
...
player_conf="${DIR}/../../source/player.conf"
config_dir="${DIR}/../../source/formations-dt"

coach_conf="${DIR}/../../source/coach.conf"
...
usage()
{
  (echo "Usage: $0 [options]"
   ...
   echo "  --matching_type MATCHING_TYPE    specifies matching algorithm used in marking"
   echo "                                   MATCHING_TYPE is one of [MinSum|MinMax]"    ) 1>&2
}
  ...
  case $1 in
	...
	--matching_type)
        ...
        case "${matching_type}" in
            MinSum)
                matching_opt="--matching_type 0"
                ;;

            MinMax)
                matching_opt="--matching_type 1"
                ;;
            *)
                usage
                exit 1
...
if [ X"${debug_log_dir}" = X'' ]; then
    mkdir -p "${DIR}/../../logs/debug/"
    debug_log_dir="${DIR}/../../logs/debug/"
fi

debugopt="${debugopt} --log_dir ${debug_log_dir}"

opt="--player-config ${player_conf} --config_dir ${config_dir}"
opt="${opt} -h ${host} -p ${port} -t ${teamname}"
opt="${opt} ${fullstateopt}"
opt="${opt} ${matching_opt}"
